...
class Runner:
    def __init__(self, args):
        ...
        # Create env
        self.env_evaluate = C3V1_Test(
            initial_xyzs=np.array([[-0.5, 0, 0.1], [0.5, 0, 0.1], [0, -0.5, 0.1]]),
            gui=Env_gui, num_drones=args.N_drones, obs=ObservationType('kin_target'),
    ...
    def evaluate_policy(self, eval_plot, eval_time):  # 仅测试一次的
        ...
        # 将数据转换为numpy数组
        if eval_plot:
            all_target_pos = 5*np.array(all_target_pos)
            all_states = 5*np.array(all_states)
            # all_actions = np.array(all_actions)
            all_rewards = 5*np.array(all_rewards)
    ...
    def plot_and_save_results(self, time_i, states, rewards, target_pos, Job_done, Success, create_html=False):
        ...
        # 保存路径数据的目录
        save_dir = f"./agent_paths/测试{self.env_name}_{self.mark}"
        ...
        with open(save_file_path, 'w') as f:
            ...
            for i in range(len(target_pos)):
                f.write(f"{target_pos[i, 0]:.3f}, {target_pos[i, 1]:.3f}, {target_pos[i, 2]:.3f}\n")
            ...
            for agent_id in range(self.args.N_drones):
                ...
                for i in range(len(agent_states)):
                    # 保留 3 位小数
                    f.write(f"{agent_states[i, 0]:.3f}, {agent_states[i, 1]:.3f}, {agent_states[i, 2]:.3f}\n")
        ...
        if create_html:
            ...
            # 保存为HTML
            html_path = os.path.join(save_dir, f"{time_i}_作战{Success}.html")
